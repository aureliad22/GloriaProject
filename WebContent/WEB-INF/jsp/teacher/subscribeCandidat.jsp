...
<h1> Inscription du candidat : ${candidat.firstName}  ${candidat.lastName} </h1>
					...
					<p> Choisir Test : </p>
										...
										<input type="submit" class="btn btn-default" name="Ajouter" value="Ajouter" onclick="ajouterTest()"/>
						...
						<p> Liste de tous les tests du candidats : </p>
								<select multiple class="form-control" id="listeTestsInscrits" name="listeTestsInscrits">					        
									<c:if test="${!empty listeTestsInscrits}">
										<c:forEach items="${listeTestsInscrits}" var="listeTestsInscrits">
											    <option value="${listeTestsInscrits.id}" > ${listeTestsInscrits.title} </option>	
										</c:forEach>
			        				</c:if>        			
					 			 </select>
			     ...
			     <script type='text/javascript'>
				        function ajouterTest(){	
							...
							<input type="submit" class="btn btn-default" name="Supprimer" value="Supprimer" /> </br></br>
							<input type="submit" class="btn btn-default" name="Valider" value="Valider l'inscription"/> 

</div>

</div>

<a href="<%=request.getContextPath()%>/Teacher/CandidateGestion"> Retour Page Candidat </a>
